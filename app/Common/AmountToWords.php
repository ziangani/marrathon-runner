...
class AmountToWords {
    ...
    public static function readAsZMW($amount){
        $amountInWordsObj = new self();
        $totalAmountPaid = number_format($amount, 2, '.', '');

        list($kwacha, $ngwee) = explode('.', $totalAmountPaid);
        $ngweeInWords = '';

        if ($ngwee > 0) {
            $ngweeInWords = ' And ' . $amountInWordsObj->toText($ngwee, 'Ngwee');
        }

        $amountInWords = $amountInWordsObj->toText($kwacha) . $ngweeInWords;
        return $amountInWords;
    }

    public static function readAsDollar($amount){
        $amountInWordsObj = new self();
        $totalAmountPaid = number_format($amount, 2, '.', '');

        list($dollar, $cent) = explode('.', $totalAmountPaid);
        $centInWords = '';

        if ($cent > 0) {
            $centInWords = ' And ' . $amountInWordsObj->toText($cent, 'Cent');
        }

        $amountInWords = $amountInWordsObj->to_text($dollar) . $centInWords;
        return $amountInWords;
    }

    public function toText($amt, $currency = 'Kwacha') {
        if (is_numeric($amt)) {
            $sign = $amt > 0 ? '' : 'Negative ';
            return $sign . $this->toQuadrillions(abs($amt)) . ' ' . $currency;
        } else {
            throw new Exception('Only numeric values are allowed.');
        }
    }

    public function to_text($amt, $currency = 'Dollar') {
        if (is_numeric($amt)) {
            $sign = $amt > 0 ? '' : 'Negative ';
            return $sign . $this->toQuadrillions(abs($amt)) . ' ' . $currency;
        } else {
